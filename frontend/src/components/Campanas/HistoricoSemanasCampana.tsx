...
export default function HistoricoSemanasCampana({ 
  ...
  const manejarGuardar = async (forzarReemplazo: boolean = false) => {
    ...
    if (!tieneMetricasTrafficker && !tieneMetricasDueno) {
      notify.error(' Debes ingresar al menos una métrica (trafficker o dueño)');
      return;
    }
    ...
    if (tieneMetricasTrafficker) {
      ...
      if (clics > alcance) {
        notify.warning(' Los clics no pueden ser mayores que el alcance (es un funnel: Alcance → Clics → Leads)');
        return;
      }
      
      if (leads > clics) {
        notify.warning(' Los leads no pueden ser mayores que los clics (es un funnel: Alcance → Clics → Leads)');
        return;
      }
    }
  ...
  const manejarEliminarSemana = async (semanaISO: number) => {
    ...
    if (!datosExistentes) {
      notify.error(' No hay datos para eliminar para esta semana');
      return;
    }
    ...
    try {
      ...
      if (resultado.exito) {
        notify.success(` Métricas de la semana ${semanaISO} eliminadas exitosamente`);
      ...
      notify.error(` Error eliminando métricas: ${error}`);
    } finally {
      setEliminando(null);
    }
  };
