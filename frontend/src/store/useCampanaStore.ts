...
function convertirFormularioAApi(formulario: FormularioCrearCampana): Omit<CampanaApi, 'id' | 'fechaCreacion' | 'fechaActualizacion' | 'semanaISO'> {
  return {
    nombre: '', // Se generará en el backend
...
export const useCampanaStore = create<CampanaStore>((set, get) => ({
  ...
  crearCampana: async (datos: FormularioCrearCampana, _idPersonalizado?: string) => {
    try {
      const datosApi = convertirFormularioAApi(datos);
